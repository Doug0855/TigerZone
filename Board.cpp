...
std::pair<int,int> Board::getOptimalPlacement(Tile &tile, std::vector< std::pair<int, int> > availableMoves) {
	for (size_t z = 0; z < availableMoves.size(); z++)
	{
		...
		int rotate = 0;
		//std::cout << "<" << i << ", " << j << ">" << std::endl;	//debugging

		//std::cout << "Address of bottom 'tile': " << (m_board[i+1][j] == NULL) << std::endl;  //debugging
		//std::cout << "Address of top 'tile': " << (m_board[i-1][j] == NULL) << std::endl;			//debugging
		//std::cout << "Address of right 'tile': " << (m_board[i][j+1] == NULL) << std::endl;		//debugging
		//std::cout << "Address of left 'tile': " << (m_board[i][j-1] == NULL) << std::endl;		//debugging
		...
		while (rotate < 4) {
				...
				(m_board[i][j - 1] == NULL || m_board[i][j - 1]->getRightFace()->faceEquals(*tile.getLeftFace())))
			{
				...
				std::cout << "return triggered" << std::endl;
			}
			else
			{
				tile.rotate();
